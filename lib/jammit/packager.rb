module Jammit
  ...
  class Packager
    ...
    def pack_individual_asset(package, extension, asset)
      ...
      if url_map.include?(asset)
        case path = url_map[asset]
        when Jammit::COFFEE_EXTENSION_MATCHER
          @compressor.compile_coffee([path])
        when Jammit.template_extension_matcher
          @compressor.compile_jst([path], package_paths.grep(Jammit.template_extension_matcher))
        else
          @compressor.concatenate([path])
        end
      else
        raise PackageNotFound, "#{extension.to_s.upcase} package \"#{package}\" does not contain an asset named \"#{asset}\"."
      end
    end
    ...
    # every request.
    def create_packages(config, extension)
      packages = {}
