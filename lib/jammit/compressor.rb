  ...
  class Compressor
    ...
    def compress_js(paths)
      ...
      js = paths.map do |path|
        if Jammit::COFFEE_EXTENSION_MATCHER =~ path
          compile_coffee([path])
        elsif Jammit.pack_templates_in_order and Jammit.template_extension_matcher =~ path
          compile_jst([path], jst_paths)
        elsif not Jammit.template_extension_matcher =~ path
          read_binary_file(path)
        end
      end.compact.join("\n")

      js += compile_jst(jst_paths) unless Jammit.pack_templates_in_order
      Jammit.compress_assets ? @js_compressor.compress(js) : js
    end

    # Compiles a single coffee file by writing out the corresponding
    # javascript generated by the coffee-script compiler.
    def compile_coffee(paths)
      unless defined?(CoffeeScript)
        raise "CoffeScript is not defined. Please add coffee-script to your Gemfile."
      end

      compiled = paths.grep(Jammit::COFFEE_EXTENSION_MATCHER).map do |path|
